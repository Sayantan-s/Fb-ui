...
const FindGiphy = () => {
    ...
    const { loading, data, error } = useFetch({
        url: `https://api.giphy.com/v1/gifs/${query ? 'search' : 'trending'}?api_key=${
            process.env.REACT_APP_API_KEY
        }&limit=20&q=${query}`, //9jtsTxgs6gXcfI2wAtjdlGYIEXMtm3l9`
    ...
    return (
        <View className="widget">
            <Input
                type="text"
                placeholder="Search gif across the world..."
                value={input}
                          ...
                          <Stack>
                              {data.map((gif, id) => (
                                  <Image
                                      ...
                                      src={gif}
                                      alt={`gif_${id}`}
                                      className="widget_gifwrapper--img"
                                      onClick={() => selectGifUrl(gif)}
                                  />
                              ))}
                          </Stack>
                      )}
